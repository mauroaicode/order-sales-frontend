        ...
        },
        {
          ...
          renderBodyCell: ({row, column, rowIndex}, h) => {
            return row.roles.map((item) => {
              return <span
                class="bg-blue-100 text-blue-800 text-xs font-medium mr-2 px-2.5 py-0.5 rounded dark:bg-blue-900 dark:text-blue-300">{item.name}</span>
            })
          },
        },
            ...
            let html = <div class="flex">
              ...
              <div>
                <button
                  ...
                  type="button"
                  class="text-white bg-red-600 hover:bg-red-500 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-md text-sm p-2.5 text-center inline-flex items-center mr-2 ">
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                       stroke="currentColor" class="w-4 h-4">
                    <path stroke-linecap="round" stroke-linejoin="round"
  ...
  methods: {
    getUsers() {
      this.$axios.get('api/v1/get-users').then(resp => {
        ...
        this.users.length > 0 ?  this.exist = false : this.exist = true
      }).catch(e => {
        this.exist = true
        console.log(e)
